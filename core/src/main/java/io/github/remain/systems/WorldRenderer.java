...
public class WorldRenderer {
    ...
    public void render(SpriteBatch batch, float cameraX, float cameraY, int selectedX, int selectedY, int selectedZ) {
        ...
        // Désactiver temporairement la grille avec texture
        // renderGrid(batch);
    ...
    private void sortBlocksForIsometricRendering(List<Block> blocks) {
        blocks.sort((b1, b2) -> {
            // 1. Toujours trier par hauteur Y en premier (bas vers haut)
            int yCompare = Integer.compare(b1.getY(), b2.getY());
            if (yCompare != 0) return yCompare;

            // 2. Appliquer la même transformation de rotation que pour l'affichage
            ...
            // 3. Tri isométrique INVERSÉ sur coordonnées transformées
            // Après transformation, on trie de l'avant vers l'arrière (inversé)
